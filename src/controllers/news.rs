...
pub async fn sql_news_id(pool: &Pool<MySql>, id: i32) ->  Result<LasterLists, Error> {
	sqlx::query_as::<_, LasterLists>(
		r#"
		select 
		ny.id,
		ny.href,
		...
		nyd.local_src,
		nyd.src,
		nyd.content from news ny, details nyd where ny.id = nyd.news_id and ny.id = ?;
		"#,
		)
		.bind(id)
